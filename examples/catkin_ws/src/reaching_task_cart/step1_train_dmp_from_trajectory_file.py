...
def main():
    ...
    for n_bfs in n_bfs_list:
        ...
        ts = traj.ts
        xs_ana, xds_ana, _, _, q_traj = dmp.analytical_solution(ts)
        traj_reproduced_ana = dmp.states_as_trajectory(ts, xs_ana, xds_ana)
        cart_traj_reproduced_ana = np.column_stack((traj_reproduced_ana.ys, q_traj))

        mae = np.mean(abs(traj.ys - np.column_stack((traj_reproduced_ana.ys, q_traj))))
        ...
        cart_traj_reproduced = np.column_stack((traj_reproduced.ys, q_steps))

        for i in range(7):
            plt.figure(i+1)
            plt.plot(traj.ys[:,i], label="demonstrated")
            plt.plot(cart_traj_reproduced_ana[:,i], label="reproduced ana")
            plt.plot(cart_traj_reproduced[:,i], label="reproduced")
            plt.legend()
            plt.show()
