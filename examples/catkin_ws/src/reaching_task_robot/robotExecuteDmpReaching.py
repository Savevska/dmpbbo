...
class DmpExecution:
  def __init__(self, args):
    ...
    self._joint_limits = joint_limits_urdf.get_joint_limits(key="/robot_description")
    self.limits_scaling_factor = 0.95
  ...
  def make_trj_to_point(self, names, positions, velocities, accelerations, freq):
      ...
      for i, name in enumerate(names):
        if positions[i] < self.limits_scaling_factor*self._joint_limits[name]["min_position"]:
          positions[i] = self.limits_scaling_factor*self._joint_limits[name]["min_position"]
        elif positions[i] > self.limits_scaling_factor*self._joint_limits[name]["max_position"]:
          positions[i] = self.limits_scaling_factor*self._joint_limits[name]["max_position"]
