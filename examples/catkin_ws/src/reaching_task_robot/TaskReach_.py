...
class TaskReach(Task):
    ...
    def evaluate_rollout(self,cost_vars,sample):
        ...
        n_misc = 15#self.traj_demonstrated_.dim_misc()#n_misc = 5
        n_time_steps = cost_vars.shape[0]
        
        # ts = cost_vars[:,0]
        joint_states = cost_vars[:,0:n_dims]
        dmp_acc = cost_vars[:, 5*n_dims:6*n_dims]
        ...
        if (dmp_acc[-1] != 100).all():
            acc_cost = np.sum(dmp_acc**2)
        else:
            acc_cost = 10000
        # costs sum
        costs = np.zeros(1+4)
        ...
        costs[4] = np.sum(self.acc_weight_*acc_cost)
