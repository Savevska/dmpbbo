...
from dmpbbo.functionapproximators.FunctionApproximatorRBFNrot import FunctionApproximatorRBFNrot

class CartDmp(DynamicalSystem, Parameterizable):
    ...
    def __init__(self, tau, y_init, y_attr, function_approximators, n_bfs, **kwargs):
        ...
        self._phase_system_rot = kwargs.get("phase_system_rot", phase_system_rot_default)
        ...
        #### ROTATION DMP ####
        # self.fa_rot = FunctionApproximatorRBFNrot(n_bfs, 0.75)
        self.rot_trained = False
        self.alpha = 48.0 #kwargs.get("alpha_spring_damper", 20.0)
    ...
    def integrate_step_quaternion(self, x, y, z, dt):
        ...
        dq = np.exp(dt / 2 * z / self.tau)
        y = dq * y

        return x, y, z


    def fix_quaternion_sign(self, quat_array):
    ...
    def from_traj(cls, trajectory, function_approximators, **kwargs):
        ...
        # ASSUMING A SEPARATE TRAJECTORY FOR EACH END EFFECTOR (assuming that the trajectory contains poses for one ee [px, py, pz, qx, qy, qz, qw]) 
        # Relevant variables from trajectory
        tau = trajectory.ts[-1]
        # Variables from the positional part of the trajectory
    ...
    def analytical_solution(self, ts=None):
        ...
        # print("      Phase analitical solution.. ", xs_phase, xds_phase)
        # Compute gating term
        xs_gating, xds_gating = self._gating_system.analytical_solution(ts)
        # print("      Gait analitical solution.. ", xs_gating, xds_gating)
        ...
        return xs, xds, forcing_terms, fa_outputs, q_traj
    ...
    def _compute_targets(self, trajectory):
        ...
        # print("    Solving analitical solution...")
        xs_ana, xds_ana, forcing_terms, fa_outputs, _ = self.analytical_solution(trajectory.ts)
    ...
    def plot(self, ts=None, xs=None, xds=None, **kwargs):
        ...
        if ts is None:
            ts = self._ts_train
        if xs is None:
            xs, xds, forcing_terms, fa_outputs, _ = self.analytical_solution(ts)
        else:
    ...
    def plot_comparison(self, trajectory, **kwargs):
        ...
        xs, xds, _, _, _ = self.analytical_solution(ts)
